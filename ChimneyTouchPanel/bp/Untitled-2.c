...
#define TICKS_PER_HEARTBEAT 49     // ~25ms
...
void sendCommand(unsigned char cmd) {  writeByte(cmd);  }
...
/*************  ✨ Windsurf Command ⭐  *************/
/**
 * @brief Turn off a specific LED segment on the grid.
 *
 * @param seg Segment number (0-7)
/*******  0447ce11-ccf9-4cda-bf51-3af3a571cfda  *******/
void turn_off_led(unsigned char seg, unsigned char grid) {
    if (seg >= 8 || grid >= 16) return;
    displayBuffer[grid] &= ~(1 << seg);
}
