...
void init_STM(void);
void init_PTM_for_tick(void);  // For system ticks
void init_CTM_for_delay(void); // New: For microsecond delays
void updateDisplay(void);
void turn_on_led(unsigned char seg, unsigned char grid);
void ctm_delay_us(unsigned int us); // New: Replacement for timer_delay_us using CTM
...
// New: Initialize CTM for microsecond delays
void init_CTM_for_delay(void) {
    _ctmc0 = 0b00000000;  // CTCK=000 (fSYS=8MHz), CTON=0
    _ctmc1 = 0b11000001;  // Mode=11 (Timer), CTCCLR=1 (clear on CCRA)
    _ctmae = 0;           // Interrupt disabled
}

// New: Microsecond delay using CTM (blocking)
void ctm_delay_us(unsigned int us) {
    unsigned int ticks = 8 * us;  // 8 MHz → 0.125 µs per tick, so 8 ticks per µs
    _ctmal = ticks & 0xFF;        // Load compare A low
    _ctmah = ticks >> 8;          // Load compare A high
    _ctmdl = 0;                   // Reset counter low
    _ctmdh = 0;                   // Reset counter high
    _cton = 1;                    // Start CTM
    while (!_ctmaf);              // Poll compare-A flag
    _ctmaf = 0;                   // Clear flag
    _cton = 0;                    // Stop CTM
}

void start(void) { _pc6 = 1; _pc7 = 1; ctm_delay_us(5); _pc7 = 0; ctm_delay_us(5); _pc6 = 0; ctm_delay_us(5); }
void stop(void) { _pc6 = 0; _pc7 = 0; ctm_delay_us(5); _pc6 = 1; ctm_delay_us(5); _pc7 = 1; ctm_delay_us(5); }
void writeByte(unsigned char data) { unsigned char i; for (i = 0; i < 8; i++) { _pc6 = 0; ctm_delay_us(5); if (data & 0x01) { _pc7 = 1; } else { _pc7 = 0; } ctm_delay_us(5); _pc6 = 1; ctm_delay_us(5); data >>= 1; } }
...
void USER_PROGRAM_C_INITIAL(void) {
    ...
    configure_io_pins();
    init_STM();
    init_PTM_for_tick();  // For system ticks
    init_CTM_for_delay(); // New: For microsecond delays
    // Enable interrupts
